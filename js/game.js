//add current action on every key down
//remove on every key up
		...
		L1: {
			playerObject: { //generate attributes object for a player
				...
				update: function() {
					...
					//switch (this.currentAction) { //handle implementation to conclusion of each action
					//case 'jumping':
					if (this.y <= canvas.height / 1.5) { //if we get too high
						this.vy = 3; //start falling down again (canvas in inverted in y so positive means going down)
					} else if (this.y >= canvas.height - 181) { //when we're back where we jumped from
			...
			handleEvents: function(player) { //player as param because its undefined as of yet
				$(document).keydown(function(e) {
					...
					if (player.currentAction !== 'jumping' || e.keyCode === 32) {
						...
						var action = player.numToAction(e.keyCode);
						...
						if (!inArray(player.currentActions, action)) {
							...
							console.log(player.currentActions);
						}
					}
				});
				$(document).keyup(function(e) {
					...
					if (player.y === canvas.height - 181 && (e.keyCode === 39 || e.keyCode === 37)) {
						player.action('standing'); //stop player
					}
					//remove action associated with key
					removeFromArray(player.currentActions, player.numToAction(e.keyCode));
					if (e.keyCode === 39 || e.keyCode === 37) player.lastAction = null;
				});
			}
		},
		//...continue levels
...
function startPlaying() {
	...
	///////////////////////////////////////////////////////////////////////////////////
	var Projectile = function(attrs){
		GameElement.call(this, attrs);
	};
	Projectile.prototype = Object.create(GameElement.prototype); //inherit GameElement and such
